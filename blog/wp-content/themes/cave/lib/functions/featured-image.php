...
add_action( 'init', 'cave_add_image_sizes' );
function cave_add_image_sizes() {
	add_image_size( 'cave-image-featured', 640, 410, true );
}

/** Cave Get Image Id Manual */
function cave_get_image_id_manual( $num = 0 ) {
...
/** Cave Get Image Id Auto */
function cave_get_image_id_auto( $num = 0 ) {
	
	/** Global Object */
	global $post;
	
	/** Manual Check At Priority */
	$id = cave_get_image_id_manual( $num );
...
/** Cave Get Image*/
function cave_get_image( $args = array() ) {
	...
	$id = '';
	if( $args['mode'] == 'manual' ) {		
		$id = cave_get_image_id_manual( $args['num'] );			
	} else {	
		$id = cave_get_image_id_auto( $args['num'] );	
	}
